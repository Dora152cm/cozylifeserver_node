class User {
  ...
  addUserSQL() {
    let sql = `INSERT INTO CUSTOMER(CustomerName, CustomerUsername, CustomerPassword, CustomerSex, CustomerBirthday, CustomerMail, CustomerTel, CustomerAddress, CustomerLogin, CustomerJoinDate) \
  ...
  updateUserByIdSQL(id) {
    let sql = `UPDATE CUSTOMER \
               SET CustomerName = '${this.name}', CustomerUsername = '${this.username}', CustomerPassword = '${this.password}', CustomerSex = '${this.sex}', CustomerBirthday = '${this.birthday}', CustomerMail = '${this.email}', CustomerTel = '${this.tel}', CustomerAddress = '${this.address}', CustomerLogin = ${this.login} \
               WHERE CustomerID =  ${id}`
    return sql
  }
  ...
  static getUserByQuerySQL(query) {
    ...
    if (query.CustomerAddress) where.push(`CustomerAddress = '${query.CustomerAddress}'`)
